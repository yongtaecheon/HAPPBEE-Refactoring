...
function HospitalLoading() {
  return (<div className='hospital-loading'>
...
function HospitalList() {
    ...
    let [lat, lon] = [0, 0];
    await new Promise((resolve: PositionCallback) => navigator.geolocation.getCurrentPosition(resolve)) //비동기 Geolocation API 정보 - 위도/경도 GET
  ...
  const { data: hospitalList, isPending } = useQuery({
    queryKey: ['hospital'],
    queryFn: fetchHospitalWithCurLocation,
  });

  return isPending ? (<HospitalLoading />)
  ...
  return (
    ...
    <div className='hospital-container'>
      <div className='hospital'>
          <h1>당신이 힘들 때, 우리는 여기 있어요.<br />어떤 어려움이든 함께 극복해 나가요.</h1>
      </div>
        <HospitalList/>
    </div>
    </section>
